...
| 2 | 02A–C | Secrets, Git và xử lý lịch sử | Lịch sử cũ đã làm sạch; bản production ngày 15/09 và migration mới chưa commit/push |
| 3 | 05A–C | Supabase: RLS, Storage, admin và Auth | RLS/Storage/profile guard và ma trận khách-user A-user B-admin đã đạt; lỗi reload lần gửi recovery đầu đã sửa trên production; còn backup Supabase và kiểm thử email/OTP thật |
...
| 7 | 06A–C | Metadata, trang riêng tư, robots/sitemap/404 | Source verifier đạt; production 404 đúng; còn crawl/metadata/canonical toàn site |
| 8 | 07 | HTML từng dự án và cơ chế xuất bản | 126 trang dự án trong public build; còn chứng minh publish ≤5 phút, unpublish và slug cũ 301 trên production |
...
| 10 | 09A–B | QA staging/production và go-live | Go-live và QA role/data/upload/download đã đạt; form recovery production xử lý ngay lần gửi đầu; còn email/OTP thật, browser/device, crawl và rollback |
...
Ưu tiên hiện tại: chốt commit/tag hoặc mốc Git cho đúng bản production đang chạy, sau đó kiểm tra callback/recovery production rồi crawl/monitoring/backup vận hành. Ma trận bảo mật, Nginx và hiệu năng lab đã đạt; có thể tiếp tục sửa giao diện sau khi lưu mốc source hiện tại.
...
- **Tiến độ 2026-09-15:** production trả 200 cho `/trang-chu`, 404 cho URL sai và các đường dẫn nội bộ nhạy cảm. Verifier source đạt; còn crawl toàn bộ sitemap và kiểm metadata/canonical/status thực tế.
...
- **Tiến độ 2026-09-15:** public build hiện có 126 trang dự án và verifier publication đạt. Chưa có bằng chứng production cho độ trễ publish ≤5 phút, unpublish, rollback và redirect slug cũ.
...
- **Tiến độ 2026-09-15:** website đã go-live; ma trận role A/B/admin, dữ liệu riêng và upload/download đã đạt trên production. Luồng từ đăng nhập tới recovery đã được sửa để khởi tạo bằng điều hướng đầy đủ và xử lý ngay lần gửi đầu; kiểm tra production với dữ liệu email không hợp lệ không reload và hiện thông báo tại chỗ. Các khoảng trống chính còn lại là email/OTP thật, browser/device, crawl, publish/unpublish và diễn tập rollback.
